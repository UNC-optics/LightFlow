...
def normalize_energy(x):
    ...
    '''
    assert x.ndim == 5, "Dimensionalities don't seem to match. This function\
...
def zeropad_to_1080p(image):
    ...
    pad_a = np.ceil((target_shape - shape) / 2.).astype(np.int)
    pad_b = np.floor((target_shape - shape) / 2.).astype(np.int)
    
    return np.pad(image, pad_width = ((0,0),(0,0),(pad_b[0], pad_a[0]),(pad_b[1], pad_a[1]),(0,0)), constant_values = 0)
