...
class Amplitude(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        if self.sum_ints:
            return tf.sqrt(tf.reduce_sum(tf.square(tf.abs(inputs)), axis = 1, keepdims = False))
        else:
...
class NormalizePhase(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        xx = tf.math.angle(tf.math.exp(tf.complex(0., inputs))) + np.pi
        if self.zero_one:
            return xx / (2*np.pi)
        else:
            return xx
...
class NormalizeEnergy(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        energy = tf.complex(tf.cast(tf.sqrt(tf.reduce_sum(tf.abs(tf.square(inputs)), axis=[2, 3], keepdims=True)), tf.float32), 0.)
...
class LightCompatible(tf.keras.layers.Layer):
    def __init__(self, list_compat = None, dims_compat = -1, name = 'GOCompatible'):
        ...
        '''
        assert list_compat in ['amplitude','phase', None], 'list compatibility not correct.\
            The tensor is either phase or amplitude, provided: {}'.format(list_compat)
        assert dims_compat > -2 and isinstance(dims_compat, int), 'invalid dimension\
            provided: {}'.format(dims_compat)
