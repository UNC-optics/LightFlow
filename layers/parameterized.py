...
class SLM(tf.keras.layers.Layer):
    ...
    def binarize(self, x):
        ...
        def grad(dy):
            ...
            return dy * (alpha/2.) * (1 - sig**2 - (1 - sig)**2)
        return bnrz, grad
    
    
    def nomralize2pi(self, x):
        return tf.math.angle(tf.exp(tf.complex(0., x)))
    
    
    def build(self, input_shape):
        shape = self.bs_nf + input_shape[0][2:-1] + (1,) # we have to ensure the last dim is 1
        
        if self.device == 'LCoS':
            self.phase = self.add_weight("Phase",
                                         shape = shape,
                                         dtype = tf.float32,
                                         trainable = True)
            self.amplitude = tf.constant(tf.ones(shape,
                                                 dtype = tf.float32))
        
        else:
            self.phase = tf.constant(tf.zeros(shape,
                                              dtype = tf.float32))
            self.amplitude = self.add_weight("Amp",
                                             shape = shape,
                                             dtype = tf.float32,
                                             trainable = True,
    ...
    def call(self, inputs):
        ...
        if self.passthrough:
            a = tf.reduce_sum(inputs[0] - inputs[0] + inputs[1] - inputs[1],
                              axis = -1,
                              keepdims = True) # we have to ensure the last dim is 1. TODO not ideal!
