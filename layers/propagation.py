...
class Propagation(tf.keras.layers.Layer):
    ...
    def __get_functions(self):
        if self.unfiltered:
            self.mask = (np.sinc(np.pi*self.ps[0]*self.big_fxx)*np.sinc(np.pi*self.ps[1]*self.big_fyy)).astype(np.complex64)
    ...
    def __get_freqMeshGrid(self):
        if self.unfiltered:
            fx = np.fft.fftshift(np.fft.fftfreq(self.shape[-2]*self.unfiltered, d = self.ps[0]))
            fy = np.fft.fftshift(np.fft.fftfreq(self.shape[-3]*self.unfiltered, d = self.ps[1]))
    ...
    def build(self, input_shape):
        assert input_shape[0] == input_shape[1], 'Phase and Amplitude dimensions mismatch.'
        ...
        if self.unfiltered:
            self.H = [np.pad(h, [[self.shape[-3]]*int((self.unfiltered-1)/2),
                                 [self.shape[-2]]*int((self.unfiltered-1)/2)]) * self.mask for h in self.H]
...
class Lens(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        shape = input_shape[0]
        psx = np.abs(self.f*self.physics['wavelength']/(shape[-2]*self.physics['pixel_size'][0]))
        psy = np.abs(self.f*self.physics['wavelength']/(shape[-3]*self.physics['pixel_size'][1]))
